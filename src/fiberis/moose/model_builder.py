...
class ModelBuilder:
    ...
    It includes a robust, data-driven mesh generation system for complex,
    multi-fracture simulations based on a stitched mesh approach. -- Shenyao
    """
    ...
    def _dump_recursive_to_string(self, block: 'MooseBlock', lines: List[str], prefix: str = ""):
        ...
        lines.append(block_path_str)
        for name, value in block.params.items():
            lines.append(f"{prefix}  . {name} = {value}")

        for sub_block in block.sub_blocks:
            self._dump_recursive_to_string(sub_block, lines, prefix=f"{prefix}  ")

    def __str__(self) -> str:
        """
        Returns a string representation of the entire model structure, showing all
        block paths and their parameters.
        """
        lines = [f"--- MOOSE Model Structure for: {self.project_name} ---"]
        for block in self._top_level_blocks:
            self._dump_recursive_to_string(block, lines)
        lines.append("-----------------------------")
        return "\n".join(lines)
    ...
    # --- Input File Generation and State Saving ---
    ...
    def save_npz(self, filepath: str):
        """
        Saves the current state of the ModelBuilder to an .npz file.
        This allows for saving a configured model baseline and reloading it later.
        The entire object is pickled and stored inside the .npz archive.

        :param filepath: The path to the .npz file where the state will be saved.
        """
        if not filepath.endswith('.npz'):
            filepath += '.npz'
        # We are saving the entire object state via pickling into a numpy array.
        np.savez(filepath, builder_state=np.array([self], dtype=object))
        print(f"ModelBuilder state successfully saved to: {filepath}")

    # Load from .npz, might be useful for complex model setups
    # However, we can not change the
    @staticmethod
    def load_npz(filepath: str) -> 'ModelBuilder':
        """
        Loads a ModelBuilder instance from a saved .npz state file.

        :param filepath: The path to the .npz file containing the saved state.
        :return: A new ModelBuilder instance with the loaded state.
        """
        with np.load(filepath, allow_pickle=True) as data:
            builder = data['builder_state'][0]
            if not isinstance(builder, ModelBuilder):
                raise TypeError(f"File '{filepath}' does not contain a valid ModelBuilder state.")
        print(f"ModelBuilder state successfully loaded from: {filepath}")
        return builder

    # --- Debugging and Visualization ---
    def plot_geometry(self, save_path: Optional[str] = None, hide_legend: Optional[bool] = False, equal_aspect: Optional[bool] = False, **kwargs) -> None:
        """
    ...
    def extract_geometry(self) -> Dict[str, Any]:
        """
        Extracts and returns the geometry information of the PDE problem.

        This method aggregates all relevant geometry details, including mesh bounds,
        SRV zones, hydraulic fractures, and nodesets, into a single dictionary.

        Returns:
            Dict[str, Any]: A dictionary containing the geometry information.
        """
        geometry_output = {
            'mesh': self.geometry_info.get('mesh', {}),
            'srv_zones': self.geometry_info.get('srv_zones', []),
            'hydraulic_fractures': self.geometry_info.get('hydraulic_fractures', []),
            'nodesets': self.geometry_info.get('nodesets', []),
            'postprocessors': self.postprocessor_info.get('postprocessors', [])
        }
