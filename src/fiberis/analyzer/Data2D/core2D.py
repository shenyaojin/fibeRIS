...
class Data2D:
    ...
    def to_moose_reporter_str(self,
                              coord_x: Optional[float] = None,
                              coord_y: Optional[float] = None,
                              coord_z: Optional[float] = None,
                              precision: int = 6) -> str:
        """
        Formats the data into a string suitable for a MOOSE OptimizationData Reporter block.

        This function generates the 'measurement_points', 'measurement_time', and
        'measurement_values' strings required by MOOSE. It expands the 1D 'daxis'
        into 3D coordinates by using it for the dimension that is left as None.

        Args:
            coord_x (Optional[float]): Fixed value for the X-coordinate. If None, daxis is used for X.
            coord_y (Optional[float]): Fixed value for the Y-coordinate. If None, daxis is used for Y.
            coord_z (Optional[float]): Fixed value for the Z-coordinate. If None, daxis is used for Z.
            precision (int): Number of decimal places for floating-point numbers in the output.

        Returns:
            str: A formatted multi-line string for direct use in a MOOSE input file.

        Raises:
            ValueError: If data/axes are not set, or if the coordinate arguments are invalid
                        (i.e., not exactly one is None).
        """
        if self.data is None or self.taxis is None or self.daxis is None:
            raise ValueError("Data, taxis, and daxis must be set before generating the MOOSE string.")

        # Validate coordinate inputs
        coords = {'x': coord_x, 'y': coord_y, 'z': coord_z}
        none_coords = [key for key, val in coords.items() if val is None]

        if len(none_coords) != 1:
            raise ValueError(f"Exactly one coordinate (x, y, or z) must be None to use daxis, "
                             f"but found {len(none_coords)}: {none_coords}")

        daxis_dim = none_coords[0]
        self.history.add_record(f"Generating MOOSE reporter string with daxis mapped to '{daxis_dim}' coordinate.",
                                level="INFO")

        n_depth = len(self.daxis)
        n_time = len(self.taxis)

        # 1. Generate measurement_points
        point_coords = []
        for d_val in self.daxis:
            coords[daxis_dim] = d_val
            point_str = f"{coords['x']:.{precision}f} {coords['y']:.{precision}f} {coords['z']:.{precision}f}"
            point_coords.append(point_str)

        # Repeat the block of spatial points for each time step
        all_points = point_coords * n_time
        points_str = '   '.join(all_points)

        # 2. Generate measurement_time
        # Repeat each time value for every spatial point
        all_times = np.repeat(self.taxis, n_depth)
        times_str = '   '.join(f"{t:.{precision}f}" for t in all_times)

        # 3. Generate measurement_values
        # Flatten data in Fortran (column-major) order to get all depth values for t1, then t2, etc.
        all_values = self.data.flatten(order='F')
        values_str = '   '.join(f"{v:.{precision}f}" for v in all_values)

        # Assemble the final string
        output = (
            f"    # MOOSE Reporter block generated by fibeRIS on {datetime.datetime.now().isoformat()}\n"
            f"    # Data from object: {self.name or 'Unnamed'}\n"
            f"    # Total measurements: {n_depth * n_time} ({n_depth} locations x {n_time} time steps)\n"
            f"    measurement_points = '{points_str}'\n\n"
            f"    measurement_time   = '{times_str}'\n\n"
            f"    measurement_values = '{values_str}'"
        )
